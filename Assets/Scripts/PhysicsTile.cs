...
public class PhysicsTile : MonoBehaviour
{
  ...
  public ShadowCaster2D tileShadowPrefab;

  private bool hasSetShadows = false;

  // private Texture2D originalTileTexture;
  // private bool hasInitialized = false;

  public void Regenerate(int column, int row)
  {
    ...
    var pixelSurface = GetComponentInParent<PixelSurface>();
    var texture = pixelSurface.GetTileTexture(column, row);
    // var texture = pixelSurface.nonLivePixelTileTex[column, row];
    var sprite = Sprite.Create(texture, new Rect(0, 0, texture.width, texture.height), new Vector2(0.5f, 0.5f), 64);


    // if (!hasInitialized)
    // {
    //   originalTileTexture = pixelSurface.GetTileTexture(column, row);
    //
    //   hasInitialized = true;
    // }
    // Debug.Log(originalTileTexture.GetPixels().Select(color => color.a).ToList());


    // float[] tileTextureToAlphas(Texture2D texturea) => texturea.GetPixels().Select(color => color.a).ToArray();
    //
    // var alphasA = tileTextureToAlphas(originalTileTexture);
    // var alphasB = tileTextureToAlphas(currentTileTexture);
    // for (int i = 0; i < alphasA.Length; i++)
    // {
    //   var a = alphasA[i];
    //   var b = alphasB[i];
    //   
    //   if (a!=b) Debug.Log("NOT THE SAME");
    // }
    ...
    for (int p = 0; p < paths.Count; p++)
    {
      ...
      if (!hasSetShadows)
      {
        var tileShadow = Instantiate(tileShadowPrefab, transform, false);
        // tileShadow.m_ShapePath = paths[p].Select(vertice => (Vector3)vertice).ToArray();
      }
    }
...
// [CustomEditor(typeof(PhysicsTile))]
...
//     {
//       PhysicsTile collider = (PhysicsTile)target;
//       collider.Regenerate();
//     }
//     base.OnInspectorGUI();
//   }
